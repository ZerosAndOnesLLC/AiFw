...
fn next_serial() -> String {
    ...
    let serial: u64 = u64::from_be_bytes([
...
pub async fn generate_ca(
    ...
    let _ = sqlx::query("DELETE FROM ca_root WHERE id = 1")
        .execute(&state.pool)
        .await;
    sqlx::query(
...
pub async fn get_ca_cert_pem(State(state): State<AppState>) -> Result<String, StatusCode> {
...
pub async fn list_certs(
    ...
    let certs: Vec<CertRecord> = rows
        .into_iter()
        .map(|(id, ct, cn, sans, serial, nb, na, status, ra, ca)| {
            let actual_status = if status == "active" && na < Utc::now().to_rfc3339() {
                "expired".to_string()
            } else {
                status
            };
...
pub async fn download_cert(
    ...
    let (cert_pem,) =
        sqlx::query_as::<_, (String,)>("SELECT cert_pem FROM ca_certificates WHERE id = ?1")
            .bind(&id)
...
pub async fn download_cert_key(
    ...
    let (key_pem,) =
        sqlx::query_as::<_, (String,)>("SELECT key_pem FROM ca_certificates WHERE id = ?1")
            .bind(&id)
            .fetch_one(&state.pool)
            .await
            .map_err(|_| StatusCode::NOT_FOUND)?;
...
pub async fn revoke_cert(
    ...
    Ok(Json(MessageResponse {
        message: format!("Certificate {id} revoked"),
    }))
}
...
pub async fn delete_cert(
    ...
    Ok(Json(MessageResponse {
        message: format!("Certificate {id} deleted"),
    }))
}

pub async fn get_crl(State(state): State<AppState>) -> Result<String, StatusCode> {
