        ...
        if let Some(ref mut iface) = current {
            ...
            if trimmed.starts_with("inet ") {
                ...
                if let Some(idx) = parts.iter().position(|&p| p == "netmask")
                    && let Some(nm) = parts.get(idx + 1)
                {
                    iface.ipv4_netmask = Some(nm.to_string());
                }
            }
...
pub async fn set_interface_role(
    ...
    Ok(Json(MessageResponse {
        message: format!("{} set as {}", name, req.role),
    }))
}
...
pub async fn delete_interface_role(
    ...
    sqlx::query("DELETE FROM interface_roles WHERE interface_name=?1")
        ...
        .map_err(|_| internal())?;
    Ok(Json(MessageResponse {
        message: format!("Role removed from {}", name),
    }))
}
...
pub async fn configure_interface(
    ...
    if let Some(ref mode) = req.ipv4_mode {
        if !["dhcp", "static", "none"].contains(&mode.as_str()) {
            return Ok(Json(MessageResponse {
                message: "Invalid IPv4 mode. Must be dhcp, static, or none.".to_string(),
            }));
        }
        if mode == "static" {
            if let Some(ref addr) = req.ipv4_address {
                ...
                return Ok(Json(MessageResponse {
                    message: "Static mode requires an IPv4 address (e.g. 192.168.1.1/24)."
                        .to_string(),
                }));
            }
        }
    ...
    {
        return Ok(Json(MessageResponse {
            message: format!("Invalid gateway: {}", e),
        }));
    }
    ...
    {
        return Ok(Json(MessageResponse {
            message: "MTU must be between 68 and 9000.".to_string(),
        }));
    }
                ...
                if let Some(ref gw) = req.gateway {
                    if !gw.is_empty() {
                        ...
                        if owns_default {
                            ...
                            msgs.push(format!(
                                "Gateway {} noted (default route stays on {})",
                                gw,
                                cur_gw_iface.as_deref().unwrap_or("unknown")
                            ));
                        }
                    }
...
fn validate_ip(ip: &str) -> Result<(), String> {
    ...
    for (i, octet) in octets.iter().enumerate() {
        let val: u16 = octet
            .parse()
            .map_err(|_| format!("Invalid octet '{}' in address.", octet))?;
        if val > 255 {
            return Err(format!(
...
pub async fn apply_vlans(pool: &SqlitePool) -> Result<(), String> {
    ...
    let db_vlan_names: Vec<String> = rows
        .iter()
        .map(|(vid, ..)| format!("vlan{}", vid))
        .collect();
...
pub async fn list_vlans(
    ...
    let vlans: Vec<VlanConfig> = rows
        .into_iter()
        .map(
            |(id, vid, parent, mode, ip4, ip6, mtu, en, desc, ca)| VlanConfig {
                id,
                vlan_id: vid as u16,
                parent,
                ipv4_mode: mode,
...
async fn validate_vlan_req(
    req: &CreateVlanRequest,
) -> Result<ValidatedVlan, (StatusCode, Json<MessageResponse>)> {
        ...
        && !ifaces.iter().any(|i| i == &parent)
    {
        return Err(bad_req(format!(
            "Parent interface '{}' does not exist on this host.",
            parent
        )));
    }
    ...
    let ipv4_address = match mode.as_str() {
        "static" => {
            let addr = req
                ...
                .filter(|s| !s.is_empty())
                .ok_or_else(|| {
                    bad_req("Static mode requires an IPv4 address (e.g. 192.168.1.1/24).")
                })?;
        ...
        .filter(|s| !s.is_empty())
    {
        Some(v6) => {
            if v6.parse::<std::net::Ipv6Addr>().is_err() {
                ...
                let (addr, prefix) = v6
                    .split_once('/')
                    .ok_or_else(|| bad_req("Invalid IPv6 address."))?;
                if addr.parse::<std::net::Ipv6Addr>().is_err() {
                    return Err(bad_req("Invalid IPv6 address."));
                }
                let p: u8 = prefix
                    .parse()
                    .map_err(|_| bad_req("Invalid IPv6 prefix length."))?;
                if p > 128 {
                    return Err(bad_req("IPv6 prefix length must be between 0 and 128."));
                }
            }
            Some(v6.to_string())
        }
...
pub async fn create_vlan(
    ...
    {
        let existing =
            sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM vlans WHERE vlan_id = ?1")
                .bind(req.vlan_id as i64)
                .fetch_one(&state.pool)
                .await
                .unwrap_or(0);
        if existing == 0 {
            return Err(bad_req(format!(
                "Device name '{}' is already in use on this host.",
                vlan_name
            )));
        }
    }
...
pub async fn delete_vlan(
    ...
    sqlx::query("DELETE FROM vlans WHERE id = ?1")
        .bind(&id)
        .execute(&state.pool)
        .await
        .map_err(|_| internal())?;
    ...
    Ok(Json(MessageResponse {
        message: format!(
            "VLAN {} marked for deletion — click Apply to remove",
            vlan_name
        ),
    }))
}
