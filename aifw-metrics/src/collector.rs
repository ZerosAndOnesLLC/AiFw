    ...
    pub async fn collect_once(&mut self) {
        match self.pf.get_stats().await {
            Ok(stats) => {
                let _ = self
                    .store
                    .record(names::PF_STATES, stats.states_count as f64)
                    .await;
                let _ = self
                    .store
                    .record(names::PF_PACKETS_IN, stats.packets_in as f64)
                    .await;
                let _ = self
                    .store
                    .record(names::PF_PACKETS_OUT, stats.packets_out as f64)
                    .await;
                let _ = self
                    .store
                    .record(names::PF_BYTES_IN, stats.bytes_in as f64)
                    .await;
                let _ = self
                    .store
                    .record(names::PF_BYTES_OUT, stats.bytes_out as f64)
                    .await;
                let _ = self
                    .store
                    .record(names::PF_RULES_COUNT, stats.rules_count as f64)
                    .await;
                let _ = self
                    .store
                    .record(names::PF_RUNNING, if stats.running { 1.0 } else { 0.0 })
                    .await;

                // Compute rates (delta / interval)
                let interval_secs = self.interval.as_secs_f64().max(1.0);
                let pps_in =
                    (stats.packets_in.saturating_sub(self.prev_packets_in)) as f64 / interval_secs;
                let pps_out = (stats.packets_out.saturating_sub(self.prev_packets_out)) as f64
                    / interval_secs;
                let bps_in = (stats.bytes_in.saturating_sub(self.prev_bytes_in)) as f64 * 8.0
                    / interval_secs;
                let bps_out = (stats.bytes_out.saturating_sub(self.prev_bytes_out)) as f64 * 8.0
                    / interval_secs;
        ...
        match self.pf.get_states().await {
            Ok(states) => {
                ...
                let icmp = states
                    .iter()
                    .filter(|s| s.protocol.starts_with("icmp"))
                    .count();
