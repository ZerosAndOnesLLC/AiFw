...
pub async fn update_status(
    ...
    let pkg_out = Command::new("sudo").args(["pkg", "upgrade", "-n"]).output().await
    ...
    let pending_os = Command::new("sudo").args(["freebsd-update", "updatesready"]).output().await
        .map(|o| o.status.success()).unwrap_or(false);

    let needs_reboot = std::path::Path::new("/var/run/reboot-required").exists()
        || Command::new("sudo").args(["freebsd-update", "updatesready"]).output().await
            .map(|o| o.status.success()).unwrap_or(false);
...
pub async fn check_updates(
    ...
    // Check pkg updates
    let pkg_result = Command::new("sudo").args(["pkg", "update"]).output().await;
    ...
    // Check OS updates
    let os_result = Command::new("sudo").args(["freebsd-update", "fetch", "--not-running-from-cron"]).output().await;
...
pub async fn install_updates(
    ...
    // Install pkg updates
    let pkg_result = Command::new("sudo").args(["pkg", "upgrade", "-y"]).output().await;
    match pkg_result {
        Ok(o) => {
            let stdout = String::from_utf8_lossy(&o.stdout);
    ...
    // Install OS updates
    let os_result = Command::new("sudo").args(["freebsd-update", "install"]).output().await;
    match os_result {
        Ok(o) => {
            if o.status.success() {
...
pub async fn reboot_system() -> Result<Json<MessageResponse>, StatusCode> {
    ...
    let _ = Command::new("sudo").args(["shutdown", "-r", "+10s", "AiFw scheduled reboot for updates"]).output().await;
