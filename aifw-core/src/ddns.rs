...
pub async fn load_config(pool: &SqlitePool) -> DdnsConfig {
    ...
    .ok()
    .flatten()
    .map(|(p, u4, u6)| DdnsConfig {
        poll_interval_secs: p.max(60),
        ip_echo_url_v4: u4,
        ip_echo_url_v6: u6,
    })
    .unwrap_or_default()
...
pub async fn save_config(pool: &SqlitePool, c: &DdnsConfig) -> Result<(), String> {
    ...
    crate::net_safety::validate_outbound_url(&c.ip_echo_url_v4)
        .await
        .map_err(|e| format!("ip_echo_url_v4: {e}"))?;
    crate::net_safety::validate_outbound_url(&c.ip_echo_url_v6)
        .await
    ...
    match record.source {
        IpSource::Explicit => {
            ...
            if want_v6 != ip.is_ipv6() {
                return Err(format!(
                    "explicit_ip family doesn't match record_type (got {:?})",
                    ip
                ));
            }
            Ok(ip)
...
async fn public_ip(url: &str, want_v6: bool) -> Result<IpAddr, String> {
    ...
    if want_v6 != ip.is_ipv6() {
        return Err(format!(
            "ip-echo returned wrong family: got {ip} (wanted v{})",
            if want_v6 { 6 } else { 4 }
        ));
    }
    Ok(ip)
...
pub async fn update_record(pool: &SqlitePool, record_id: i64) -> Result<UpdateOutcome, String> {
    let mut record = load_record(pool, record_id)
        .await
        .ok_or_else(|| format!("ddns record {record_id} not found"))?;
    if !record.enabled {
        return Ok(UpdateOutcome::Skipped("record disabled".into()));
    }
    let provider = crate::acme::load_provider(pool, record.provider_id)
        .await
    ...
    if matches!(record.record_type, RecordType::A | RecordType::Both) {
        match detect_ipv4(&record, &cfg).await {
            Ok(ip) => {
                ...
                } else if let Err(e) = writer
                    .upsert_a(&record.hostname, ip, record.ttl as u32)
                    .await
                {
        ...
        match detect_ipv6(&record, &cfg).await {
            Ok(ip) => {
                ...
                if prev == Some(ip.to_string().as_str()) {
                    ...
                    .upsert_aaaa(&record.hostname, ip, record.ttl as u32)
                    .await
                {
