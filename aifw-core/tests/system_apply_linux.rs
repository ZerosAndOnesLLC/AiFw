...
use aifw_core::system_apply::{
    ApplyReport, BannerInput, ConsoleInput, GeneralInput, SshInput, apply_banner, apply_console,
    apply_general, apply_ssh, collect_info,
};
...
async fn linux_apply_banner_is_noop_ok() {
    let r = apply_banner(&BannerInput {
        login_banner: "hi".into(),
        motd: "there".into(),
    })
    .await;
    assert!(r.ok);
