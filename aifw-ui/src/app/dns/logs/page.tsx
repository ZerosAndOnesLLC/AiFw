    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3">
        <input
          ...
          placeholder="Search logs (domain, IP, NXDOMAIN, SERVFAIL...)"
        ...
        <div className="overflow-y-auto max-h-[calc(100vh-280px)] p-1">
          {loading ? (
            ...
            <div className="text-center py-8 text-[var(--text-muted)]">
              {search ? "No matching log entries" : "No DNS query logs yet. Enable query logging in the DNS Resolver settings."}
            </div>
          ) : (
            <div className="font-mono text-xs space-y-px">
              {logs.map((line, i) => {
                const level = getLogLevel(line);
                const isDnsEvent = /NXDOMAIN|SERVFAIL|REFUSED|NOERROR|query|reply/.test(line);
                return (
                  <div
                    key={i}
                    className={`px-3 py-0.5 rounded hover:bg-gray-800 ${levelColor(level)} ${isDnsEvent ? "bg-gray-800/50" : ""}`}
                  >
                    {line}
                  </div>
                );
