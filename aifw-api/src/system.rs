...
use aifw_core::system_apply::{
    ApplyReport, BannerInput, ConsoleInput, GeneralInput, SshInput, SystemInfo, apply_banner,
    apply_console, apply_general, apply_ssh, collect_info,
};
use aifw_core::system_apply_helpers::{
    validate_baud, validate_domain, validate_hostname, validate_ssh_port,
};
...
pub async fn get_general(State(state): State<AppState>) -> Result<Json<GeneralDto>, StatusCode> {
    ...
    let timezone = get_kv(&state.pool, "timezone")
        .await
...
pub async fn get_banner(State(state): State<AppState>) -> Result<Json<BannerDto>, StatusCode> {
    let login_banner = get_kv(&state.pool, "login_banner")
        .await
        .unwrap_or_default();
...
pub async fn put_banner(
    ...
    if req.login_banner.len() > MAX || req.motd.len() > MAX {
        return Err((
            ...
            "banner/motd must be ≤ 8 KiB".into(),
        ));
    }
    set_kv(&state.pool, "login_banner", &req.login_banner).await;
    set_kv(&state.pool, "motd", &req.motd).await;
    let report = apply_banner(&BannerInput {
        login_banner: req.login_banner,
        motd: req.motd,
    })
    .await;
    Ok(Json(report))
...
pub async fn get_ssh(State(state): State<AppState>) -> Result<Json<SshDto>, StatusCode> {
    let enabled = get_kv(&state.pool, "ssh_enabled")
        .await
        .map(|v| v == "true")
        .unwrap_or(true);
    let port = get_kv(&state.pool, "ssh_port")
        .await
        .and_then(|v| v.parse().ok())
        .unwrap_or(22);
    let password_auth = get_kv(&state.pool, "ssh_password_auth")
        .await
        .map(|v| v == "true")
        .unwrap_or(false);
    let permit_root_login = get_kv(&state.pool, "ssh_permit_root_login")
        .await
        .map(|v| v == "true")
        .unwrap_or(false);
    Ok(Json(SshDto {
...
pub async fn put_ssh(
    ...
    set_kv(
        &state.pool,
        "ssh_password_auth",
        if req.password_auth { "true" } else { "false" },
    )
    .await;
    set_kv(
        &state.pool,
        "ssh_permit_root_login",
        if req.permit_root_login {
            "true"
        } else {
            "false"
        },
...
pub async fn get_console(State(state): State<AppState>) -> Result<Json<ConsoleDto>, StatusCode> {
    let kind_str = get_kv(&state.pool, "console_kind")
        .await
    ...
    let baud = get_kv(&state.pool, "console_baud")
        .await
        .and_then(|v| v.parse().ok())
        .unwrap_or(115200);
...
pub async fn put_console(
    ...
    let report = apply_console(&ConsoleInput {
        kind: req.kind,
        baud: req.baud,
    })
    .await;
...
fn enumerate_timezones() -> Vec<String> {
    ...
    if !out.iter().any(|z| z == "UTC") {
        out.push("UTC".to_string());
    }
    out.sort();
    out.dedup();
    out
