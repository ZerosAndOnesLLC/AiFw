...
export default function ReverseProxyLogsPage() {
  ...
  const getLogLevel = (line: string): string => {
    if (line.includes("ERROR") || line.includes("FATAL") || line.includes(" 5")) return "error";
    if (line.includes("WARN") || line.includes(" 4")) return "warn";
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)]">TrafficCop reverse proxy access logs</p>
        </div>
      ...
      <div className="flex gap-3">
        <input
          ...
          placeholder="Search logs (URL, status code, IP...)"
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-lg overflow-hidden">
        <div className="px-4 py-2 border-b border-gray-700 flex items-center justify-between">
          <span className="text-xs text-[var(--text-muted)]">
            {logs.length} log entries · /var/log/trafficcop/access.log
          </span>
          {autoRefresh && (
