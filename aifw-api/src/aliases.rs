...
pub async fn get_alias(
    ...
    let alias = state.alias_engine.get(uuid).await.map_err(|_| StatusCode::NOT_FOUND)?;
...
pub async fn create_alias(
    ...
    let alias = state.alias_engine.add(alias).await.map_err(|_| bad_request())?;
...
pub async fn update_alias(
    ...
    let alias = state.alias_engine.update(alias).await.map_err(|_| bad_request())?;
    state.set_pending(|p| p.firewall = true).await;
    Ok(Json(ApiResponse { data: alias }))
}
...
pub async fn delete_alias(
    ...
    let uuid = Uuid::parse_str(&id).map_err(|_| bad_request())?;
    state.alias_engine.delete(uuid).await.map_err(|_| StatusCode::NOT_FOUND)?;
    state.set_pending(|p| p.firewall = true).await;
    Ok(Json(MessageResponse { message: "Alias deleted".to_string() }))
}
