  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Time Service Logs</h1>
          <p className="text-sm text-[var(--text-muted)]">rTIME NTP/PTP activity log</p>
        </div>
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 text-xs text-[var(--text-muted)] cursor-pointer">
            <input type="checkbox" checked={autoRefresh} onChange={(e) => setAutoRefresh(e.target.checked)} className="rounded border-gray-600" />
            Auto-refresh
          </label>
          <button onClick={fetchLogs} className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-white text-xs rounded-md transition-colors">Refresh</button>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
          placeholder="Search logs (NTP, PTP, offset, source...)"
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-lg overflow-hidden">
        <div className="px-4 py-2 border-b border-gray-700 flex items-center justify-between">
          <span className="text-xs text-[var(--text-muted)]">{logs.length} log entries &middot; /var/log/rtime/rtime.log</span>
          {autoRefresh && (
        ...
        <div className="overflow-y-auto max-h-[calc(100vh-280px)] p-1">
          {loading ? (
            ...
            <div className="text-center py-8 text-[var(--text-muted)]">
              {search ? "No matching log entries" : "No rTIME logs yet. Start the time service to see activity."}
            </div>
          ) : (
            <div className="font-mono text-xs space-y-px">
              {logs.map((line, i) => {
                ...
                return (
                  <div key={i} className={`px-3 py-0.5 rounded hover:bg-gray-800 ${levelColor(level)}`}>{line}</div>
                );
              })}
            </div>
          )}
